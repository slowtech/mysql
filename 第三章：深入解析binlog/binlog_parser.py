...
conn = pymysql.connect(host=host, user=user, passwd=passwd, port=port, charset='utf8', autocommit=False)
cursor = conn.cursor()
    ...
    if isinstance(binlog_event, QueryEvent) and binlog_event.query == "BEGIN":
        ...
        if isinstance(binlog_event, DeleteRowsEvent):
            for each_row in rows:
                col_name = ' AND '.join(
                    ['`%s`=%%s' % (k) if v != None else '`%s` is %%s' % (k) for k, v in each_row['values'].items()])
                delete_sql = "DELETE FROM `{0}`.`{1}` WHERE {2};".format(table_schema, table_name, col_name)
                print(cursor.mogrify(delete_sql, list(each_row['values'].values())))
            ...
            for each_row in rows:
                ...
                insert_sql = 'INSERT INTO `{0}`.`{1}` ({2}) VALUES ({3});'.format(table_schema, table_name, col_name,
                                                                                  format_str)
                print(cursor.mogrify(insert_sql, list(each_row['values'].values())))
        elif isinstance(binlog_event, UpdateRowsEvent):
            for each_row in rows:
                before_values = each_row["before_values"]
                after_values = each_row["after_values"]
                set_str = ','.join(
                    ['`%s`=%%s' % (k) for k, v in after_values.items()])
                where_str = ' AND '.join(
                    ['`%s`=%%s' % (k) if v != None else '`%s` is %%s' % (k) for k, v in before_values.items()])
